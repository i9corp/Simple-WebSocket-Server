...
namespace SimpleWeb {
    ...
    public:
        ...
        private:
            ...
            Message(unsigned char fin_rsv_opcode, std::size_t length) noexcept : std::istream(&streambuf),
        ...
        public:
            Connection(std::unique_ptr<socket_type> &&socket) noexcept : socket(std::move(socket)), timeout_idle(0),
        ...
        private:
            ...
            Connection(std::shared_ptr<ScopeRunner> handler_runner, long timeout_idle, Args &&... args) noexcept
                    : handler_runner(std::move(handler_runner)), socket(new socket_type(std::forward<Args>(args)...)),
                      timeout_idle(timeout_idle), strand(socket->get_io_service()), closed(false) {
                this->uniqueId = this->generateUniqueId(this);
            }
            ...
            long generateUniqueId(void *reference) {
                char mBuffer[32];
                memset(mBuffer, '\0', 32);
                snprintf(mBuffer, 32, "%p", reference);

                long mId = strtol(mBuffer, NULL, 16);
                return mId;
            }
            ...
            public:
                SendData(std::shared_ptr<SendStream> header_stream, std::shared_ptr<SendStream> message_stream,
                        ...
                        : header_stream(std::move(header_stream)), message_stream(std::move(message_stream)),
            ...
            void send_from_queue() {
                ...
                strand.post([self]() {
                    asio::async_write(*self->socket, self->send_queue.begin()->header_stream->streambuf,
                                      self->strand.wrap(
                                              [self](const error_code &ec, std::size_t /*bytes_transferred*/) {
                                                  ...
                                                  if (!ec) {
                                                      asio::async_write(*self->socket,
                                                                        self->send_queue.begin()->message_stream->streambuf.data(),
                                                                        self->strand.wrap([self](const error_code &ec,
        ...
        public:
            std::function<void(std::shared_ptr<Connection>, long)> on_open;
            std::function<void(std::shared_ptr<Connection>, std::shared_ptr<Message>, long)> on_message;
            std::function<void(std::shared_ptr<Connection>, int, const std::string &, long)> on_close;
            std::function<void(std::shared_ptr<Connection>, const error_code &, long)> on_error;
            std::function<void(std::shared_ptr<Connection>, long)> on_ping;
            std::function<void(std::shared_ptr<Connection>, long)> on_pong;
    ...
    protected:
            ...
            asio::async_read_until(*connection->socket, connection->read_buffer, "\r\n\r\n",
                                   [this, connection](const error_code &ec, std::size_t /*bytes_transferred*/) {
                                       ...
                                       if (!ec) {
                                           ...
                                           if (RequestMessage::parse(stream, connection->method, connection->path,
                                                                     connection->query_string, connection->http_version,
                                                                     connection->header))
                                               write_handshake(connection);
                                       }
                                   });
                ...
                if (regex::regex_match(connection->path, path_match, regex_endpoint.first)) {
                    ...
                    if (connection->generate_handshake(write_buffer)) {
                        ...
                        asio::async_write(*connection->socket, *write_buffer,
                                          [this, connection, write_buffer, &regex_endpoint](const error_code &ec,
                                                                                            std::size_t /*bytes_transferred*/) {
                                              connection->cancel_timeout();
                                              auto lock = connection->handler_runner->continue_lock();
                                              if (!lock)
        ...
        void read_message(const std::shared_ptr<Connection> &connection, Endpoint &endpoint) const {
            asio::async_read(*connection->socket, connection->read_buffer, asio::transfer_exactly(2),
                             [this, connection, &endpoint](const error_code &ec, std::size_t bytes_transferred) {
                                 auto lock = connection->handler_runner->continue_lock();
                                 if (!lock)
                                     return;
                                 if (!ec) {
                                     ...
                                     if (length == 126) {
                                         ...
                                         asio::async_read(*connection->socket, connection->read_buffer,
                                                          ...
                                                          [this, connection, &endpoint, fin_rsv_opcode](
                                                              ...
                                                              if (!ec) {
                                                                  ...
                                                                  for (std::size_t c = 0; c < num_bytes; c++)
                                                                      length +=
                                                                              static_cast<std::size_t>(length_bytes[c])
                                                                                      << (8 * (num_bytes - 1 - c));

                                                                  read_message_content(connection, length, endpoint,
                                                                                       fin_rsv_opcode);
                                                              } else
        ...
        void read_message_content(const std::shared_ptr<Connection> &connection, std::size_t length, Endpoint &endpoint,
                                  unsigned char fin_rsv_opcode) const {
            if (length + (connection->fragmented_message ? connection->fragmented_message->length : 0) >
            ...
            asio::async_read(*connection->socket, connection->read_buffer, asio::transfer_exactly(4 + length),
                             [this, connection, length, &endpoint, fin_rsv_opcode](const error_code &ec,
                                                                                   std::size_t /*bytes_transferred*/) {
                                 auto lock = connection->handler_runner->continue_lock();
                                 if (!lock)
                                     return;
                                 if (!ec) {
                                     ...
                                     if ((fin_rsv_opcode & 0x80) == 0 || (fin_rsv_opcode & 0x0f) == 0) {
                                         if (!connection->fragmented_message) {
                                             connection->fragmented_message = std::shared_ptr<Message>(
                                     ...
                                     if ((fin_rsv_opcode & 0x0f) == 8) {
                                         ...
                                         if (endpoint.on_ping)
                                             endpoint.on_ping(connection, connection.get()->getUniqueId());
                                         ...
                                         if (endpoint.on_pong)
                                             endpoint.on_pong(connection, connection.get()->getUniqueId());
                                         ...
                                         if (endpoint.on_message)
                                             endpoint.on_message(connection, message, connection.get()->getUniqueId());
        ...
        void connection_open(const std::shared_ptr<Connection> &connection, Endpoint &endpoint) const {
            ...
            if (endpoint.on_open) {
                endpoint.on_open(connection, connection.get()->getUniqueId());
            }
        }

        void connection_close(const std::shared_ptr<Connection> &connection, Endpoint &endpoint, int status,
                              const std::string &reason) const {
            connection->cancel_timeout();
            connection->set_timeout();

            long mUniqueId = connection.get()->getUniqueId();
            {
                std::unique_lock<std::mutex> lock(endpoint.connections_mutex);
            ...
            if (endpoint.on_close)
                endpoint.on_close(connection, status, reason, mUniqueId);
        }

        void connection_error(const std::shared_ptr<Connection> &connection, Endpoint &endpoint,
                              const error_code &ec) const {
            connection->cancel_timeout();
            connection->set_timeout();
            ...
            if (endpoint.on_error)
                endpoint.on_error(connection, ec, connection.get()->getUniqueId());
        }
    };
